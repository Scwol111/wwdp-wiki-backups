---
title: Лаваленд
description:  огромная, случайным образом генерируемая локация, служащая в качестве шахты. Содержит в себе огромное множество различных руд, враждебной фауны, редких предметов, сокровищ и возможные локации для лута.
published: false
date: 2025-12-27T10:45:16.539Z
tags: карго, руководство
editor: markdown
dateCreated: 2025-03-15T21:03:41.485Z
---

Лаваленд – бескрайная, процедурно генерируемая пустошь на вулканической планете, приблизившейся за тысячи лет к своей звезде чуть ближе положенного. В своих недрах скрыты несметные залежи руд, подстерегают опасные твари, покоятся редчайшие артефакты и сокровища. Rock and stone! 

<h2><img style="vertical-align:middle" src="/roles/cargo/pickaxe.png"> Начало начал <img style="vertical-align:middle" src="/roles/cargo/pickaxe.png"></h2>
Прежде чем начать копать руду и рубить монстров, вам нужно снаряжение:

<div style="overflow-x:auto; margin-top: 3em;">
<figure class="table" style="text-align:center;">
  <table style="background-color:#ad5313;">
    <tbody>
      <tr>
        <th style="background-color:#ad5313;">Изображение</th>
        <th style="background-color:#ad5313;">Название</th>
        <th style="background-color:#ad5313;">Описание</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/guides/supply/lavaland/explorer_suit.png" sizes="" width="75%"></figure>
        </th>
        <th><strong>Костюм исследователя</strong></th>
        <th>Армированный костюм для работы в неблагоприятных условиях. Обеспечивает хорошую защиту от фауны, но бесполезен против бури. Укрепляется в 3 этапа пластинами голиафа по 5 штук на уровень. Можно временно заменить любым скафандром.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/gasexplorer.png" width="75%"></figure>
        </th>
        <th><strong>Противогаз исследователя</strong></th>
        <th>Брутально выглядящий противогаз, однако не более чем дыхательная маска. Ничтожно защищает от всех видов урона.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/oxygen-tank-yellow.png" width="60%" class="image"><img src="/roles/cargo/oxygen-tank-red.png" width="60%"></figure>
        </th>
        <th><strong>Кислородный баллон 

<p> 
Азотный баллон</strong></th>
        <th>
          <p>Чтобы выжить снаружи дольше пары минут, вам понадобится кислородный баллон. Экипируйте его карман (если он аварийный) или на спину, чтобы иметь возможность правильно использовать его с вашим противогазом. Важно: азотом дышат только слаймолюды.</p>
        </th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/kinetic-accelerator.png" width="60%"></figure>
        </th>
        <th><strong>Протокинетический ускоритель</strong></th>
        <th>Он и его различные вариации позволяют эффективно копать руду. Возможна установка обвесов, повышающих урон, скорострельность и дальность атаки.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/survival-knife.png" width="60%"></figure>
        </th>
        <th><strong>Нож для выживания</strong>
        <th>Малюсенький ножик для битвы с монстрами врукопашную. Нелеп тем фактом, что ранит слабее, чем простой кухонный нож, нанося всего 8 урона. Может использоваться в качестве штыка.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/pickaxe.png" width="60%"></figure>
        </th>
        <th><strong>Кирка</strong></th>
        <th>Шахтёрская кирка. Несмотря на сотни поколений старателей, она всё та же.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/mining-bag.png" width="60%"></figure>
        </th>
        <th><strong>Сумка для руды</strong></th>
        <th>Действительно специальная сумка для переноски руды. Вмещает достаточно много. Оснащена встроенным магнитом.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/gps.png" width="60%"></figure>
        </th>
        <th><strong>Глобальная система позиционирования (ГСП)</strong></th>
        <th>Показывает текущее местоположение в виде пары координат. Рекомендуется брать с собой в каждый поход, так как легко заблудиться.</th>
      </tr>
  		<tr>
        <th>
          <figure class="image"><img src="/guides/supply/lavaland/jaunter.png" width="60%"></figure>
        </th>
        <th><strong>Джаунтер</strong></th>
        <th>В случае падения в яму, телепортирует обратно на поверхность. Расходуется после использования.</th>
      </tr>
  		<tr>
        <th>
          <figure class="image"><img src="/guides/supply/lavaland/capsule.png" width="60%"></figure>
        </th>
        <th><strong>Капсула укрытия</strong></th>
        <th>Создаёт укрытие, в котором можно пережить бурю. Работает только в Лаваленде.</th>
      </tr>
    </tbody>
  </table>
</figure>
</div>

Снаряжение можно найти как на станции, так и на аванпосте, так что ничего страшного, если вы отправитесь с голыми карманами на Лаваленд.
Если имеются шахтёрские ваучеры, то можете воспользоваться ими, чтобы получить полезные вещи. Для начала рекомендуется выбирать набор новичка, включающий в себя сумку, где собраны почти все нужные для вас вещи.
Собравшись в путь, отправляйтесь в свой отдел и ищите шахтёрский шаттл. Совершите Сверхсветовой скачок до Лаваленда.


## Шахтёрский инструмент
За годы работы наших сотрудников на рудниках, было создано множество вариаций шахтёрских инструментов, подходящих под задачи каждого сотрудника.

<div style="overflow-x:auto; margin-top: 3em;">
<figure class="table" style="text-align:center;">
  <table style="background-color:#ad5313;">
    <tbody>
      <tr>
        <th style="background-color:#ad5313;">Изображение</th>
        <th style="background-color:#ad5313;">Название</th>
        <th style="background-color:#ad5313;">Описание</th>
      </tr>
			<tr>
        <th>
          <figure class="image"><img src="/roles/cargo/pickaxe.png" width="60%"></figure>
        </th>
        <th><strong>Кирка</strong></th>
        <th>Шахтёрская кирка. Несмотря на сотни поколений старателей, она всё та же.</th>
      </tr>
      <tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/utildrill.png" width="60%"></figure>
        </th>
        <th><strong>Шахтёрский бур</strong></th>
        <th>Копает в разы быстрее кирки.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/utildrilldiamond.png" width="60%"></figure>
        </th>
        <th><strong>Алмазный шахтёрский бур</strong></th>
        <th>Копает в разы быстрее простого бура.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/kinetic-accelerator.png" width="60%"></figure>
        </th>
        <th><strong>Протокинетический ускоритель</strong></th>
        <th>Базовая вариация протокинетика. Имеет 100 ячеек вместимости.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/crusher.png" width="60%"></figure>
        </th>
        <th><strong>Крушитель</strong></th>
        <th>Ранняя вариация протокинетического ускорителя. Его болты не наносят урона, но накладывают на цель метку, усиливающую атаку. Атака становится смертоноснее, если наносить удар со спины.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/crusher_glaive.png" width="60%"></figure>
        </th>
        <th><strong>Глефа-крушитель</strong></th>
        <th>Крушитель, но тоньше и лучше.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/crusher_hammer.png" width="60%"></figure>
        </th>
        <th><strong>Кинетический молот</strong></th>
        <th>Габаритный молот, который эффективно ломает породу и кости врагов. Имеет возможность накладывать метку, но отсутсвует усиление для атак со спины.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/crusher_machete.png" width="60%"></figure>
        </th>
        <th><strong>Кинетический мачете</strong></th>
        <th>Уменьшенный, одноручный вариант крушителя, позволяющий блокировать атаки. Его болты не наносят урона, но накладывают на цель метку, усиливающую атаку. Атака становится смертоноснее, если наносить удар со спины.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/crusher_spear.png" width="60%"></figure>
        </th>
        <th><strong>Кинетическое копьё</strong></th>
        <th>Слабее обыкновенного крушителя, но бьёт куда дальше. Его болты не наносят урона, но накладывают на цель метку, усиливающую атаку. Атака становится смертоноснее, если наносить удар со спины.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/crusher_claws.png" width="60%"></figure>
        </th>
        <th><strong>Кинетические когти</strong></th>
        <th>Позволяют копать породу руками. Их болты не наносят урона, но накладывают на цель метку, усиливающую атаку. Атака становится смертоноснее, если наносить удар со спины. ОЧЕНЬ смертоносны в умелых руках.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/kinetic_pistol.png" width="60%"></figure>
        </th>
        <th><strong>Протокинетический пистолет</strong></th>
        <th>Уменьшенная версия ПКУ, стреляющая ослабленными болтами, но имеющая вместимость аж в 200 единиц.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/kinetic_repeater.png" width="60%"></figure>
        </th>
        <th><strong>Протокинетическая винтовка</strong></th>
        <th>ПКУ, но стреляет чередой болтов на короткой дистанции. Имеет вместимость в 60 единиц.</th>
      </tr>
      <tr>
        <th>
          <figure class="image"><img src="/roles/cargo/kinetic_shotgun.png" width="60%"></figure>
        </th>
        <th><strong>Протокинетический дробовик</strong></th>
        <th>ПКУ, но стреляет несколькими болтами за раз. Имеет вместимость в 60 единиц.</th>
      </tr>
    </tbody>
  </table>
</figure>
</div>

## Прокачка оружия и брони
шкура голиафа по броне тык-тык - крутая броня
модуль купить в автомате и по пушке тык-тык - крутая пушка

## Руда
Лаваленд богат разными видами руд: от угля до блюспейс кристаллов, которые могут быть полезны станции.


<h2><img style="vertical-align:middle" src="/guides/supply/lavaland/ore_processor.png"> Система очков <img style="vertical-align:middle" src="/guides/supply/lavaland/ore_processor.png"></h2>
Добыв руду, не забудьте отправить её в переработчик. Закончив плавить руды ОБЯЗАТЕЛЬНО переведите себе очки, которые пойдут на вашу ID-карту - иначе их возьмут ваши коллеги. Переплавленную руду можете сунуть в автолат, чтобы сделать её доступной для всей станции. (убедитесь, что он подключен к блюспейс хранилищу).

Полученные очки можно потратить в утильмаге, где можно достать расходники, новые шахтёрские инструменты, улучшения, броню и даже огнестрельное оружие.

## Структуры
Аванпост - настоящая крепость, которая будет вашим вторым домом. Имеется комната шахтёров, торговые автоматы, кухня, прачечная, телекомы, больница и комната КМа. Защищайте его до последнего.

Лаваленд полон и других структур: разрушенные убежища, дендрарий, разбитые шаттлы и многое другое.

Разрушенные убежища: обломки космических станций, которые не успели сгореть в атмосфере и удачно приземлились на раскалённую пустошь.

Заброшенный дендрарий: райский сад, неведомо появившийся на этой жаркой и 
пустынной планете. Возможно, он был построен первыми колонизаторами планеты, желавшими внедрить частичку жизни в это враждебное место.

Аванпост синдиката: нашим исследователям удалось обнаружить сеть биолабораторий Синдиката, где проходили разработки биологического оружия. Научный персонал покинул данные объекты, и теперь они находятся под охранной провинившихся наёмников.
## Фауна
Одному Богу известно, как без воды появились и находятся живые существа. Раскалённая поверхность, разреженный воздух и скудная растительность вынудили эволюционировать местных животных в настоящих монстров.

Список:
- Сталагмит - живое зубоподобное существо, призывающее всех остальных. Со временем может дать сундук некрополиса
- Голиаф - магматическое четвероногое существо, которое так и наровит создать свои щупальца под вашими ногами. Будьте осторожны и вовремя уклоняйтесь от его атак, чтобы не ползти до базы.
- Наблюдатель - летающий глаз, стреляющий шоковыми лучами, которые оставляют неприятные ожоги на теле. Лучшей стратегией будет спрятаться за укрытие или пойти в лобовую атаку, ибо он обладает феноменальной точностью, недоступной нашим шахтёрам.
- Магматическая рука - слайм, состоящий из слизи.

## Боссы
<center><img src="/guides/supply/lavaland/hierophant.gif" alt="Иерофант"></center>
Иерофант - загадочное существо, обитающее на своей арене.
Стратегия очень простая: уворачивайтесь от его атак (белые зоны), наносите как можно больше урона и не стойте на месте. В битве с ним пригодятся все предметы, повышающие вашу скорость: ботинки ослепляющей скорости, химикаты, броня, не сковывающая движений.

<center><img src="/guides/supply/lavaland/arena.png" alt="Арена"></center>

После убийства босса падает посох, который имеет 3 способности:
1. Колдовать X-образную атаку.
2. Ставить маркер для телепорта.
3. Телепортироваться на маркер.
<div style="overflow-x:auto; margin-top: 3em;">
<figure class="table" style="text-align:center;">
  <table style="background-color:#ad5313;">
    <tbody>
      <tr>
        <th style="background-color:#ad5313;">Изображение</th>
        <th style="background-color:#ad5313;">Название</th>
        <th style="background-color:#ad5313;">Описание</th>
      </tr>
      <tr>
        <th>
          govnoxalupa 1
        </th>
        <th>
          ataka
        </th>
        <th>
          Призывает X-образную атаку, которая наносит урон
        </th>
      </tr>
      <tr>
        <th>
          govnoxalupa 2
        </th>
        <th>
          Маркер для телепорта
        </th>
        <th>
          Ставит маркер, на который можно телепортироваться
        </th>
      </tr>
      <tr>
        <th>
          govnoxalupa 3
        </th>
        <th>
          ТП
        </th>
        <th>
          Телепортирует по координатам марки, проявляя вокруг двух точек поле.
        </th>
      </tr>
    </tbody>
  </table>
</figure>
</div>